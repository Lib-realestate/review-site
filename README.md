...
**写真も出したい場合**は `image` を追加する（楽天のリンク生成画面で「画像とテキスト」を選んだときに出てくる `<img src="...">` の中のURL）：

```yaml
products:
  - name: "メリーズ さらさらエアスルー パンツ"
    url: "https://hb.afl.rakuten.co.jp/ichiba/..."
    image: "https://hbb.afl.rakuten.co.jp/hgb/...&s=240x240"
```

**もっと簡単な方法（推奨）**：楽天のリンク作成画面でコピーした「画像とテキスト」のコードを、加工せずそのまま `raw` に貼り付けるだけでもよい。url と image を自動で読み取る。

```yaml
products:
  - name: "メリーズ さらさらエアスルー パンツ"
    raw: |
      <table border="0" cellpadding="0" ...（楽天からコピーしたコードをそのまま貼り付け）...</table>
```

`raw` を使うときは、その下に半角スペース6個分インデントして貼り付ける（YAMLの決まりなので、崩さずそのままコピペすればOK）。URLが見つからない場合はビルド時にエラーで教えてくれる。

`### 商品名` 見出しの直後に、写真＋「楽天市場で見る」ボタンのカードが自動で入る。

**比較表やどこの本文にもリンクを置きたい場合**は `{{buy:商品名}}` と書く。products の name と一致するものを自動で小さな「見る→」リンクに変換する。

```markdown
| 商品名 | 価格 | 購入リンク |
|---|---|---|
| メリーズ さらさらエアスルー パンツ | 6,617円 | {{buy:メリーズ さらさらエアスルー パンツ}} |
```

同じ `{{buy:商品名}}` は本文中に何度書いてもよい（レビュー文中・表・まとめ、どこでも同じリンクに変換される）。

## 2.7 記事のトップ画像・一覧のサムネイル

frontmatterに `heroImage` を追加すると、記事上部に大きく表示され、トップ・カテゴリ一覧のカードにもサムネイルとして自動で使われる。

```yaml
heroImage: "https://hbb.afl.rakuten.co.jp/hgb/...&s=240x240"
heroImageAlt: "画像の説明文（省略時はtitleが使われる）"
```

**画像の出どころについて**：他人の写真を許可なく使うのは著作権上のリスクがあるため、以下のいずれかを使うこと。
- 楽天の商品画像（アフィリエイトリンク作成画面で取得できるもの。最も手軽）
- 自分で撮影・用意した写真
- ライセンスが明確なフリー素材（Unsplash / Pexelsなど、商用利用可の表記があるもの）

`heroImage` を設定しない記事は、今まで通り画像なしのテキスト中心の見た目になる（壊れたりはしない）。
